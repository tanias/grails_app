	...
	<body>
		...
		<div id="show-author" class="content scaffold-show" role="main">
			...
			<ol class="property-list author">
				...
				<g:if test="${authorInstance?.micropost}">
				<li class="fieldcontain">
					<span id="micropost-label" class="property-label"><g:message code="author.micropost.label" default="Micropost" /></span>
					
						<g:each in="${authorInstance.micropost}" var="m">
						<span class="property-value" aria-labelledby="micropost-label"><g:link controller="micropost" action="show" id="${m.id}">${m?.encodeAsHTML()}</g:link></span>
						</g:each>
